...
Phork.Blazor.Reactivity is a Blazor state management library. It helps take advantage of .NET's [INotifyPropertyChanged](https://docs.microsoft.com/en-us/dotnet/api/system.componentmodel.inotifypropertychanged?view=net-6.0) and [INotifyCollectionChanged](https://docs.microsoft.com/en-us/dotnet/api/system.collections.specialized.inotifycollectionchanged?view=net-6.0) interfaces to automatically manage the state of your components.

Advantages of the library:

* You can use reactive one-way and two-way (in combination with `@bind` directive) bindings that can make the component re-render if any `INotifyPropertyChanged` instance in the binding path raises appropriate `PropertyChanged` event.
* You can additionally make your components react to `CollectionChanged` notifications of objects implementing  `INotifyCollectionChanged` interface.
...
* Although using this library can greatly simplify implementing MVVM in your components, the library does not suggest usage of any particular presentation design pattern (you can even use none if you want).
...
Add the following line to _Imports.razor_ file at the root of your Blazor project:
...
> **Note:** If your component has a direct base type other than the default `ComponentBase`, you can still take advantage of the library. All you need to do is to implement `IReactiveComponent` in your component. [This](./docs/DETAILS.md#implementing-ireactivecomponent) document will guide you through the steps.
...
* [Detailed information of observed collection](./docs/DETAILS.md#observed-collections)
...
* [Detailed information of observed values](./docs/DETAILS.md#observed-bindings)
...
2. Although the `Observed`, the `ObservedCollection`, and the `Binding` methods are generic methods, you **MUST NOT** explicitly set their generic type parameters (e.g., as a means of up-casting the returned value in the `Observed` and the `ObservedCollection`). Doing so will cease the library's ability to reuse reactivity elements. And in some cases may lead to `InvalidOperationException`. Always let the `valueAccessor` (and converters in case of converted observed binding) define the generic type parameters.
3. As the converter parameters in the `Binding` method of converted reactive bindings have `Func` types, it may seem reasonable to use lambda expressions as their values, however, doing so will disable the libraries ability to cache bindings. Always write your conversion logic in instance or static methods and pass those as converter arguments.
4. To use converted observed bindings, the `converter` and the `reverseConverter` delegates need to be inverse functions of each other for the binding logic to work.
